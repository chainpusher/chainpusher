...
var assembler *service.TronBlockChainAssembler
...
func TestTronTranserContract(t *testing.T) {
	...
	assembler = service.NewTronBlockChainAssembler()
...
func TestTronUsdtTriggerSmartContract(t *testing.T) {
	...
	smart, err := client.GetContractABI(service.TronUsdtAddress)
	if err != nil {
		t.Error(err)
	}
	...
	contract := transaction.Transaction.RawData.GetContract()[0]
	transfer, err := assembler.TransactionFromContract(&args, transaction, contract)
	if err != nil {
		t.Error(err)
	}
