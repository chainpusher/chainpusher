...
func TestTronBlockChainService_GetNowBlock(t *testing.T) {
	...
	service, err := service.NewEthereumClient(url, nil)
	if err != nil {
...
func TestEthereumGetTransactionOfUSDT(t *testing.T) {
	...
	block, err := svc.GetBlock(big.NewInt(20045824))
	if err != nil {
		t.Fatal("Failed to get block: ", err)
		return
	...
	for _, tx := range block.Transactions() {
		...
		if tx.To().String() == service.EthereumUsdtAddress {
			t.Log(tx.Value())
		}
	}
	t.Log("Transactions: ", block.Transactions().Len())
...
func TestEthBlockChain_GetNowBlock(t *testing.T) {
	...
	if key == "" {
		t.Log("Failed to get Infura key: INFURA_KEY")
		return
	}
...
func TestEthereumAbiParse(t *testing.T) {
	...
	parsedABI, err := abi.JSON(strings.NewReader(service.EthereumUsdtAbi))
	if err != nil {
...
func TestEthereumDataMethod(t *testing.T) {
	...
	if !bytes.Equal(dataBytes[:4], service.EthereumUsdtMethodTransfer) {
		t.Fatalf("Expected method: %v, got: %v", service.EthereumUsdtMethodTransfer, dataBytes[:4])
	}
}
